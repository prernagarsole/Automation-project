            ...
            if cap.isOpened():
                ...
                #     out_file = os.path.splitext(filename)[0] + '_rotateccw.mp4'
                    ...
                    for i in range(len(frames)):
                        ...
                        frames[i] = cv2.GaussianBlur(frames[i], ksize, sigmaX)
                    out_file = os.path.splitext(filename)[0] + '_gaussianblur.mp4'
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    out_path = os.path.join(app.config['UPLOAD_FOLDER'], out_file)
                    out = cv2.VideoWriter(out_path, fourcc, 20.0, (frame_width, frame_height))
                    for frame in frames:
                    ...
                    out_file = os.path.splitext(filename)[0] + '_horizontalflip.mp4'
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    out_path = os.path.join(app.config['UPLOAD_FOLDER'], out_file)
                    out = cv2.VideoWriter(out_path, fourcc, 20.0, (frame_width, frame_height))
                    for frame in frames:
                ...
                elif augmentation == 'randomrotate':
                    for i in range(len(frames)):
                        angle = random.uniform(-15, 15)
                        M = cv2.getRotationMatrix2D((frame_width / 2, frame_height / 2), angle, 1)
                        frames[i] = cv2.warpAffine(frames[i], M, (frame_width, frame_height))
                    out_file = os.path.splitext(filename)[0] + '_randomrotate.mp4'
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    out_path = os.path.join(app.config['UPLOAD_FOLDER'], out_file)
                    out = cv2.VideoWriter(out_path, fourcc, 20.0, (frame_width, frame_height))
                    for frame in frames:
                    ...
                    for i in range(len(frames)):
                        frames[i] = cv2.flip(frames[i], 0)
                    out_file = os.path.splitext(filename)[0] + '_flip_vertical.mp4'
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    out_path = os.path.join(app.config['UPLOAD_FOLDER'], out_file)
                    out = cv2.VideoWriter(out_path, fourcc, 20.0, (frame_width, frame_height))
                    for frame in frames:
